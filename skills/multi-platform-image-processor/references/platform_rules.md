...
平台目录是完整流程交付包的主体。顶层业务图片位于产品交付包根目录的 `合格证`、`吊牌图` 和 `尺码图`，不放入单个平台目录。蜂享家＋爱库存模板中的平台级 `尺码图` 目录保持平台规则定义的空目录结构。
...
- 详情页使用平铺结构或完整的上/下结构。平铺结构在 `详情\静态` 直接放置图片；上/下结构同时提供非空的 `详情\静态\上` 与 `详情\静态\下`。两种结构互斥。
- `主图\1440`、`SKU\1440`、`素材图` 为可选目录。

## 详情页模块顺序

Agent 逐张检查全部详情源图，生成内部模块计划。固定顺序为：品牌背书、KV 主视觉、适用图标、产品信息、尺码表、图标说明，然后是其他卖点、面料、认证、模特、品牌故事和店铺模块。

- 六类必需模块全部存在。
- 模块计划覆盖每张详情源图，同类模块保持原相对顺序。
- 适用图标卡片与图标说明在同一张图时，从卡片下方纯背景间隔处水平拆分，卡片边框和圆角完整保留。
- 输出保持源图宽度。单张高度超过 `1600px` 时分段，片段按从上到下顺序相邻排列。
- 产品数据包保持原样，模块排序与拆分使用任务临时文件并写入交付目录。
...
平台目录名为 `产品货号 产品名称-京东`，货号与名称之间使用一个半角空格。货号和名称以唯一匹配的产品信息 Excel 为准。
...
- `800sku去除文字`：来源 `SKU` 里的 800 图。文字只能由生图模型移除，本地脚本只负责裁切、白底填充、验收和回贴。脚本从右向左识别白色商品卡片的左边缘，向左增加图片宽度 `3%` 的安全余量，截取右侧纵向区域；该区域放入与原图同尺寸、同宽高比的纯白色模型输入画布。模型只处理商品卡片彩色装饰条内的文字。模型结果恢复到模型输入尺寸后，只对右侧裁片执行差异验收，明显变化比例上限为 `15%`、平均通道差异上限为 `10`，标签内部非文字底图变化比例上限为 `15%`。模型调用失败或结果验收不通过时自动重试一次，第二次仍失败才按原图输出并记录两次失败原因。最终只将标签内部去字区域贴回原图，并保留 `2px` 标签边缘保护带。装饰条的颜色、形状、圆角、弧度保持原样；产品图、人物、衣服印花、织标、背景、图片四角使用原图。没有可识别装饰条或没有文字时直接按原图压缩输出并记录风险。最终输出 `800x800 JPG`，单张小于 `500KB`。
