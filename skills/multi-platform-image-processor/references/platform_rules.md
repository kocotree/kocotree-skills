...
- 每张透明图必须为具有实际透明背景的 PNG。明显的多个独立区域可共同组成商品主体，极小独立区域视为脏点，大小无法自动判断的区域视为问题并停止处理。
- 透明图检测阈值统一定义在 `scripts/config/透明图规则.json`，由管理员维护通用判定边界。
...
- 产品数据包保持原样，模块排序与拆分发生在本地工作副本和交付目录。
