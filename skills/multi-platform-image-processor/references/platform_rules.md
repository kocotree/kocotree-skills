...
- 每张透明图必须为具有实际透明背景的 PNG。透明度不超过 `8` 的像素按透明处理；明显的多个独立区域可共同组成商品主体，极小独立区域视为脏点，大小无法自动判断的区域需要文件规则确认。
- 可在 `透明图\透明图规则.json` 中为图片设置允许的最大主体区域数。图片路径相对于透明图目录，格式示例：
  ```json
  {
    "文件规则": {
      "颜色.png": {
