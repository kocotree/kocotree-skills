...
1. 使用多维表当前款“成分”字段中的部位、纤维名称、百分比和限定语作为商品成分展示依据；当前 SKU 行缺失成分时，使用产品信息 NAS 补充该字段。
2. 多维表检测报告附件用于核对报告样品、检测项目和宣传范围；多维表没有报告附件时，使用检测报告 NAS 补充。
...
- `material_reference_path`：多维表记录定位信息，或 NAS 补充文件的完整路径。
...
多维表无法访问时先恢复用户权限。多维表字段和 NAS 补充均无法取得，或不能唯一匹配当前款时，将 `material_composition_status` 填写为 `needs_evidence`，列明需要的表、记录、文件、工作表、行列、款号和版本。资料缺失不判为通过。
